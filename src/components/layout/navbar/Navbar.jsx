...
const Navbar = () => {
  ...
  // Lock scroll and pause Lenis when mobile menu is open
  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = 'hidden';
      document.documentElement.style.overflow = 'hidden';
      window.lenis?.stop(); // <-- Pause Lenis
    } else {
      document.body.style.overflow = '';
      document.documentElement.style.overflow = '';
      window.lenis?.start(); // <-- Resume Lenis
    }
    
    return () => {
      document.body.style.overflow = '';
      document.documentElement.style.overflow = '';
      window.lenis?.start(); // <-- Safety resume
    };
  }, [isOpen]);
    ...
    <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            className='lg:hidden fixed inset-0 bg-black/50 backdrop-blur-lg z-30 touch-none'
          />
        )}
      </AnimatePresence>
      ...
      >
        ...
        <div className='hidden lg:flex items-center gap-3 lg:gap-5'>
          {navItems.map((item) => (
            <button
              key={item.id}
              className='nav-option text-white/50 bg-transparent border-none cursor-pointer outline-none hover:text-white transition-colors duration-200 whitespace-nowrap'
          ...
          <button
                  ...
                  className='
                            ...
                            text-black bg-white rounded-full h-fit w-fit font-medium px-2 py-1 whitespace-nowrap
                            '
          >
            Let's Talk
          </button>
        ...
        <button 
                ...
                className='flex lg:hidden items-center justify-center w-8 h-8 rounded-full bg-white/5 border border-white/10 text-white/80 cursor-pointer hover:bg-white/10 hover:text-white transition-all duration-300'
        > 
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
                ...
                className='
                            lg:hidden absolute top-full left-0 right-0 mt-3 p-6
